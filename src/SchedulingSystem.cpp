...
bool SchedulingSystem::allProcessesDone() const
{
  ...
  // check each process in the process table
  for (int pid = 0; pid < numProcesses; pid++)
  {
    // if the process is not done, the answer is false, not all
    // processes are done yet
  ...
  // otherwise all processes in table were done, so answer is
...
void SchedulingSystem::dispatchCpuIfIdle()
{
  ...
  // if cpu is idle then ask our policy to dispatch the next process
  // to run
  if (isCpuIdle())
  {
    cpu = policy->dispatch();

    // update the start time of this process if needed
...
void SchedulingSystem::checkProcessFinished()
{
  ...
  // b. make sure you set the done flag of the process to be true
  // c. The cpu should not be IDLE because the process just
  // finished.

  // if cpu is idle then no process is running thus nothing to
  // check
  if (isCpuIdle())
  {
    return;
  ...
  // otherwise there is a process and it is now finished, so update
  // process table information to record this process finishing
